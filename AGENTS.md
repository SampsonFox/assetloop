...
- Promote only a tested `uat` commit to `prod` through a pull request, using a merge commit so `uat` remains in production ancestry. `uat` and `prod` MUST reject direct pushes, force pushes, and deletion.
- If legacy history makes `prod` not an ancestor of `uat`, align it through a dedicated `prod -> uat` pull request using a merge commit. This ancestry-only synchronization is the sole exception to squash-merging pull requests into `uat`; its resulting tree MUST preserve the already tested UAT content.
