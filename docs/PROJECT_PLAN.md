...
当前已批准增量：产品规格标签化。保留类别与型号，Item 直接归属型号；规格描述改为型号限定范围内的可选类型化标签，3D 使用已确认的外观默认规则。实施进度：领域选择校验及外观匹配规则已实现并测试，数据库迁移、应用接入和 Web 尚未完成，现有预览仍使用旧规格结构。下文关于规格父层级、颜色归属和按规格绑定行情的旧描述将在对应集成阶段统一替换；它们不是本次重构的目标约束。行情抓取不在本批次范围内。
最后更新：2026-09-05
