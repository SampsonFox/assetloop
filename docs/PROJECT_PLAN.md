...
Web 的主要用户入口是具体物品列表。类别、型号、价格规格和其他目录配置属于管理页面；AI Harness 通过 MCP 可直接调用对应应用用例，不要求用户先经过管理页面。
...
状态：已完成。累计回归覆盖 SQLite 和 PostgreSQL；UAT 发布包已手工验证类别、型号、两个价格规格、具体物品、外币买入确认、维修、更正、卖出、本位币汇总和追加式审计约束。
