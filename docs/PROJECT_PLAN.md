...
当前已批准增量：产品规格标签化。保留类别与型号，Item 直接归属型号；规格描述改为型号限定范围内的可选类型化标签，3D 使用已确认的外观默认规则。实施进度：成对 00013 迁移、Unicode 回填、事务用例、标签管理、物品型号/标签录入和资源标签/分类已接入；SQLite 迁移回滚、引用保护、HTTP 权限及共享外观模型全要素回归通过。型号外观默认的完整界面、旧入口清理和浏览器验收仍在进行，现有预览未部署这次迁移。PostgreSQL 实测待测试连接恢复后补齐。下文关于规格父层级、颜色归属和按规格绑定行情的旧描述将在对应集成阶段统一替换；它们不是本次重构的目标约束。行情抓取不在本批次范围内。
最后更新：2026-09-05
...
- 独立 3D 资源库、型号/含颜色规格的默认模型及具体物品专属模型，支持手工维护与详情交互展示。
- 跨平台单二进制发布。
...
颜色默认是具体物品属性，不拆成规格；只有确定颜色显著影响市场价格时才升级为规格维度。
...
- 分类、型号、规格和具体物品应用服务，支持非破坏性编辑和类别图标键。
- 首页具体物品列表、列表/卡片视图和空状态；物品与型号列表使用服务端筛选、排序和分页，型号页一次批量加载当前页规格；具体物品新建/编辑使用与详情一致的独立页面，列表不提供直接编辑入口，详情页是已有物品的唯一编辑入口；录入物品时可就地补充缺少的类别、型号和规格。
- 独立的类别、型号、规格管理页面；Viewer 不接收管理入口或表单。
- SQLite/PostgreSQL Store 一致性和升级测试。
- 颜色归属产品规格；规格按型号、名称、颜色区分，具体物品选择含颜色的规格。旧物品颜色迁移到对应规格，保留物品与生命周期历史。
- 独立 3D 模型库支持分页搜索、上传、预览、来源编辑、引用查询及未引用资源删除/清理重试。GLB 内容固定，更换文件创建新资源，可跨规格和物品复用。
- 型号、规格和具体物品可就地选择已有资源、上传绑定或解除绑定。详情按“物品专属 → 规格默认 → 型号通用”解析，支持旋转、缩放和重置；无模型或加载失败时回退现有图片/类别图标。解除专属绑定恢复继承。
...
验收：用户可以在 Web 中建立类别、型号、包含颜色的规格及其下的具体物品，并保持完整租户隔离；Owner/Editor 可维护资源库与三级模型绑定，所有可读角色可查看有效模型。刻字物品的专属模型不影响共用默认模型的其他物品；引用中的资源不可删除。
