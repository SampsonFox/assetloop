...
- 本期无数据库变更、手工估值或行情接入。后续市场参考区独立展示同规格/成色/地区中位数、区间、预计回收、假设卖出的净成本与日均，并附来源、样本量及更新时间；挂牌价不冒充成交价。

### 后续行情能力

- 行情序列由型号、配置标签快照、成色、市场地区和数据源共同确定；不再以旧规格 ID 为身份。
- 每日保存聚合价格点，不伪造历史回填。
- 第一个真实采样日是曲线起点。
- 某型号/配置标签快照下所有物品卖出后，仍继续刷新 90 天，再停止自动刷新（后续行情阶段）。
...
- 交易与资产事件。
- 开发分支已补充生命周期事务内校验和请求幂等：并发买入/卖出保持唯一有效记录，Web 自动生成请求标识，同一命令重试返回原事件；更正及幂等回执原子提交。此增量尚未作为新的 UAT 检查点验收。
...
验收：型号＋配置标签快照在指定成色、地区及来源下，可以生成可追溯的二手行情曲线和当前本位币估值。
