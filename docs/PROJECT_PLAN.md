...
当前已批准增量：产品规格标签化。保留类别与型号，Item 直接归属型号；规格描述改为型号限定范围内的可选类型化标签，3D 使用已确认的外观默认规则。实施进度：领域规则、成对 00013 迁移及共享 Unicode 数据回填已实现，SQLite 迁移与回滚测试通过；应用接入和 Web 尚未完成，现有预览未部署这次迁移。PostgreSQL 实测待测试连接恢复后补齐。下文关于规格父层级、颜色归属和按规格绑定行情的旧描述将在对应集成阶段统一替换；它们不是本次重构的目标约束。行情抓取不在本批次范围内。
最后更新：2026-09-05
