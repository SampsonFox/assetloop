...
当前已批准增量：产品规格标签化。保留类别与型号，Item 直接归属型号；规格描述改为可选类型化标签，3D 使用已确认外观默认。迁移 00013 已在原 8080 SQLite 数据库落地，备份、原物品/事件/资源逐字段比对和 GLB 哈希检查通过。型号允许范围、标签库、物品选择、资源描述、候选与上传绑定已接通。开发验收和文档收尾进行中；PostgreSQL 实测尚未完成，UAT 前必须补齐。本批次不接行情 API、任务调度或新 MCP 传输。
最后更新：2026-09-06
...
- 颜色及旧规格原文迁成标签，原物品和生命周期历史保持完整。已执行的 00013 不回写；后续成对前向迁移清理旧规格结构。新 Item 只接受 model_id 与 tag_ids，旧 variant_id 参数明确拒绝，不再转换。旧表和适配器清理完成前不宣称规格退休已交付。
