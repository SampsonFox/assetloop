...
func registerQueries(server *sdk.Server, s Services) {
	...
	register(server, "list_event_types", "Read system and custom event types and their enabled states. Types are reusable action categories: built-in purchase (acquiring the item, recorded once per item), repair and sale, plus user-confirmed custom cost categories such as services or accessories. Reuse a matching enabled type; individual product or service names stay in event notes.", ScopeRead, application.CapabilityView, func(ctx context.Context, p application.Principal, q TagQuery) (any, error) {
