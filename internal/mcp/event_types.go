...
type CreateEventTypeInput struct {
	...
	Name       string `json:"name" jsonschema:"Name of a reusable lifecycle action category, not a product-specific name. Reusable service and accessory cost categories are allowed once the user asks for them; individual purchased item and service names belong in event notes."`
...
func registerEventTypes(server *sdk.Server, s Services) {
	register(server, "create_event_type", "Create a reusable custom lifecycle action category only when list_event_types has no suitable enabled type and the user explicitly confirms creating a category. Reusable service and accessory cost categories are allowed once the user asks for them; individual product and service names still belong in event notes, and recording a purchase alone does not authorize catalog expansion. Reuse request_key on retries.", ScopeLifecycle, application.CapabilityManageCatalog, func(ctx context.Context, p application.Principal, q CreateEventTypeInput) (any, error) {
