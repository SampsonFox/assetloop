	...
	e = s.store.WithMarketWrite(ctx, a.TenantID, func(st MarketStore) error {
		...
		for _, existing := range items {
			if existing.Provider == item.Provider && existing.Keyword == item.Keyword && existing.FilterCriteria == item.FilterCriteria && existing.Region == item.Region {
				if existing.ModelDesc != quote.ModelDesc {
		...
		if cmd.AssetID != "" {
			if _, err = st.GetAsset(ctx, a.TenantID, cmd.AssetID); err != nil {
				return err
			}
			if err = st.BindAssetMarket(ctx, a.TenantID, cmd.AssetID, item.ID); err != nil {
				return err
			}
		}
		return nil
...
func (s *MarketService) refresh(ctx context.Context, tenant, id string) (result error) {
	...
	price := s.price(item, quote, base)
	s.convert(ctx, &price)
	return s.store.WithMarketWrite(ctx, tenant, func(st MarketStore) error {
		m, e := st.GetMarketItem(ctx, tenant, id)
		if e != nil {
			return e
		}
