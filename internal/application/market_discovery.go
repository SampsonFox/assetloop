...
func (s *MarketService) SaveMarketDiscovery(ctx context.Context, a Principal, id, name string, accept bool) (domain.MarketItem, error) {
	return s.saveMarketDiscovery(ctx, a, id, name, accept)
}
func (s *MarketService) saveMarketDiscovery(ctx context.Context, a Principal, id, name string, accept bool, commit ...func(domain.MarketItem, domain.MarketPrice) (domain.MarketItem, error)) (domain.MarketItem, error) {
	d, e := s.discovery(a, id)
	if e != nil {
	...
	item, e := s.create(ctx, a, CreateMarketItem{Name: name, Query: d.state.Query, ConfirmedModel: d.state.Quote.ModelDesc}, selectionJSON, commit...)
