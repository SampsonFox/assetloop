...
func (s *Server) renderResource(w http.ResponseWriter, r *http.Request, p application.Principal, status int, message string, draft *domain.Model3DResource) {
	...
	data := pageData{Title: resource.Name, Principal: &p, CSRFToken: s.ensureCSRF(w, r), Error: message, Resource: &resource, References: references, CanManageCatalog: p.Can(application.CapabilityManageCatalog), ReturnTo: r.URL.RequestURI()}
