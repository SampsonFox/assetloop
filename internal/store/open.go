...
func Migrate(ctx context.Context, db *sql.DB, cfg config.Database) error {
	...
	if current >= 7 && current < 12 {
		if err := checkLegacyEventTypes(ctx, db); err != nil {
			return err
		}
	}
	...
	provider, err := goose.NewProvider(dialect, db, migrationFS,
		goose.WithExcludeNames([]string{specificationMigrationName, specificationContractMigrationName, marketMigrationName, marketSelectionMigrationName, tradeInMigrationName}),
		goose.WithGoMigrations(specificationMigration(cfg.Driver), specificationContractMigration(cfg.Driver), marketMigration(cfg.Driver), marketSelectionMigration(cfg.Driver), tradeInMigration(cfg.Driver)))
	if err != nil {
	...
	if cfg.Driver == "sqlite" {
		...
		for current < target {
			if current == 10 || current == 11 || current == 12 || current == 13 || current == 20 {
