...
func TestOAuthUpgradeFrom14(t *testing.T) {
	...
	provider, err := goose.NewProvider(goose.DialectSQLite3, db, files,
		goose.WithExcludeNames([]string{specificationMigrationName, specificationContractMigrationName, "00015_oauth.sql"}),
	...
	if err := db.QueryRow("SELECT MAX(version_id) FROM goose_db_version").Scan(&version); err != nil || version != 15 {
