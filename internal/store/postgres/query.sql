...
ORDER BY
  ...
  CASE WHEN sqlc.arg(sort_key)::text = 'model' AND sqlc.arg(sort_direction)::text = 'asc' THEN LOWER(model_name) END ASC,
  CASE WHEN sqlc.arg(sort_key)::text = 'model' AND sqlc.arg(sort_direction)::text = 'desc' THEN LOWER(model_name) END DESC,
...
WHERE e.tenant_id = sqlc.arg(tenant_id) AND e.asset_id = sqlc.arg(asset_id)
  ...
  AND (sqlc.arg(show_voided)::boolean OR e.trade_in_state <> 'cancelled')
...
ORDER BY
  ...
  CASE WHEN sqlc.arg(sort_key)::text = 'type' AND sqlc.arg(sort_direction)::text = 'asc' THEN t.name END ASC,
  CASE WHEN sqlc.arg(sort_key)::text = 'type' AND sqlc.arg(sort_direction)::text = 'desc' THEN t.name END DESC,
