...
INSERT INTO asset_market_bindings(tenant_id,asset_id,market_item_id) VALUES(CAST(CAST(sqlc.arg(tenant) AS TEXT) AS UUID),CAST(CAST(sqlc.arg(asset_id) AS TEXT) AS UUID),sqlc.arg(market_item_id)) ON CONFLICT(tenant_id,asset_id) DO UPDATE SET market_item_id=excluded.market_item_id;
