...
SELECT e.trade_in_link_id AS link_id,
       ...
       CAST(CASE WHEN st.system_code = 'trade_in_source' THEN e.id::text ELSE COALESCE(c.id::text, '') END AS TEXT) AS source_event_id,
       CAST(CASE WHEN st.system_code = 'trade_in_destination' THEN e.id::text ELSE COALESCE(c.id::text, '') END AS TEXT) AS destination_event_id,
       CAST(CASE WHEN st.system_code = 'trade_in_source' THEN e.asset_id::text ELSE COALESCE(c.asset_id::text, e.related_asset_id::text, '') END AS TEXT) AS new_asset_id,
       CAST(CASE WHEN st.system_code = 'trade_in_destination' THEN e.asset_id::text ELSE COALESCE(c.asset_id::text, e.related_asset_id::text, '') END AS TEXT) AS old_asset_id,
       ...
       CASE WHEN st.system_code = 'trade_in_source' THEN FALSE WHEN ca.id IS NULL THEN TRUE ELSE FALSE END AS new_asset_deleted,
       CASE WHEN st.system_code = 'trade_in_destination' THEN FALSE WHEN ca.id IS NULL THEN TRUE ELSE FALSE END AS old_asset_deleted
FROM asset_events e
...
WHERE e.tenant_id = sqlc.arg(tenant_id)
  ...
  AND (sqlc.arg(include_cancelled)::boolean OR e.trade_in_state = 'active')
