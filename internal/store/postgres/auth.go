...
func (s *Store) ChangeMemberRole(ctx context.Context, actor application.Principal, userID string, role application.Role, event application.SecurityEvent) error {
	ids, err := authIDs(actor.TenantID, userID, actor.UserID)
	if err != nil {
		return err
	}
	tenant, target, actorID := ids[0], ids[1], ids[2]
	return s.WithManagementWrite(ctx, actor.TenantID, func(store application.ManagementStore) error {
		st := store.(*Store)
		current, err := st.queries().GetMemberRole(ctx, postgresdb.GetMemberRoleParams{TenantID: tenant, UserID: actorID})
		if err != nil {
			return err
		}
		if current != "owner" {
		...
		old, err := st.queries().GetMemberRole(ctx, postgresdb.GetMemberRoleParams{TenantID: tenant, UserID: target})
		if err != nil {
			return err
		}
		if old == string(role) {
		...
		_, err = st.queries().ChangeMemberRole(ctx, postgresdb.ChangeMemberRoleParams{TenantID: tenant, UserID: target, Role: string(role)})
		if err != nil {
			return err
		}
		event.Detail = old + " -> " + string(role)
		row, err := postgresSecurityEvent(event)
		if err != nil {
			return err
		}
		return st.queries().CreateSecurityAuditEvent(ctx, row)
	})
}
