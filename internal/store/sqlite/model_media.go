...
func (s *Store) GetModel3DBinding(ctx context.Context, tenantID, kind, targetID string) (application.Model3DBinding, error) {
	id, tenant := targetID, tenantID
	r, err := s.queries().GetModel3DBinding(ctx, sqlitedb.GetModel3DBindingParams{ID: id, TenantID: tenant, Kind: kind})
	if err != nil {
...
func resourceValue(r sqlitedb.Model3dResource) (domain.Model3DResource, error) {
	...
	return domain.Model3DResource{
		ID: r.ID, TenantID: r.TenantID, Name: r.Name, Status: r.Status, CreatedAt: created,
		ProductModel3D: domain.ProductModel3D{ResourceID: r.ID, StoreID: r.StoreID, ObjectKey: r.ObjectKey, SHA256: r.Sha256, SizeBytes: r.SizeBytes, SourceURL: r.SourceUrl, Author: r.Author, License: r.License, UpdatedAt: updated},
	}, nil
}
func (s *Store) CreateModel3DResource(ctx context.Context, r domain.Model3DResource) error {
	id, tenant := r.ID, r.TenantID
	return s.queries().CreateModel3DResource(ctx, sqlitedb.CreateModel3DResourceParams{ID: id, TenantID: tenant, Name: r.Name, Status: r.Status, StoreID: r.StoreID, ObjectKey: r.ObjectKey, Sha256: r.SHA256, SizeBytes: r.SizeBytes, SourceUrl: r.SourceURL, Author: r.Author, License: r.License, CreatedAt: sqliteTime(r.CreatedAt), UpdatedAt: sqliteTime(r.UpdatedAt)})
}
func (s *Store) GetModel3DResource(ctx context.Context, tenantID, resourceID string) (domain.Model3DResource, error) {
	...
	r, err := s.queries().GetModel3DResource(ctx, sqlitedb.GetModel3DResourceParams{ID: id, TenantID: tenant})
...
func (s *Store) ListModel3DResources(ctx context.Context, tenantID string, o application.Model3DResourceListOptions) (application.Model3DResourceListResult, error) {
	...
	rows, err := s.queries().ListModel3DResources(ctx, sqlitedb.ListModel3DResourcesParams{TenantID: tenant, SearchQuery: o.Query, PageSize: int64(o.PageSize), PageOffset: int64((o.Page - 1) * o.PageSize)})
	if err != nil {
		return application.Model3DResourceListResult{}, err
	}
	result := application.Model3DResourceListResult{Total: int(total), Resources: []domain.Model3DResource{}}
	for _, row := range rows {
		r, err := resourceValue(sqlitedb.Model3dResource{ID: row.ID, TenantID: row.TenantID, Name: row.Name, Status: row.Status, StoreID: row.StoreID, ObjectKey: row.ObjectKey, Sha256: row.Sha256, SizeBytes: row.SizeBytes, SourceUrl: row.SourceUrl, Author: row.Author, License: row.License, CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt})
		if err != nil {
			return application.Model3DResourceListResult{}, err
		}
...
func (s *Store) UpdateModel3DResource(ctx context.Context, r domain.Model3DResource) error {
	id, tenant := r.ID, r.TenantID
	n, err := s.queries().UpdateModel3DResource(ctx, sqlitedb.UpdateModel3DResourceParams{ID: id, TenantID: tenant, Name: r.Name, SourceUrl: r.SourceURL, Author: r.Author, License: r.License, UpdatedAt: sqliteTime(r.UpdatedAt)})
...
func (s *Store) Model3DReferences(ctx context.Context, tenantID, resourceID string) ([]application.Model3DReference, error) {
	id, tenant := resourceID, tenantID
	rows, err := s.queries().Model3DReferences(ctx, sqlitedb.Model3DReferencesParams{ID: nullString(id), TenantID: tenant})
	if err != nil {
		return nil, err
	}
	result := []application.Model3DReference{}
	for _, row := range rows {
		result = append(result, application.Model3DReference{Kind: row.Kind, ID: row.ID, Name: row.Name})
	}
	return result, nil
}
func (s *Store) MarkModel3DResourcePendingDelete(ctx context.Context, tenantID, resourceID string) error {
	id, tenant := resourceID, tenantID
	n, err := s.queries().MarkModel3DResourcePendingDelete(ctx, sqlitedb.MarkModel3DResourcePendingDeleteParams{ID: id, TenantID: tenant})
	if err != nil {
		return err
	}
	if n == 0 {
...
func (s *Store) FinishModel3DResourceDelete(ctx context.Context, tenantID, resourceID string) error {
	id, tenant := resourceID, tenantID
	_, err := s.queries().FinishModel3DResourceDelete(ctx, sqlitedb.FinishModel3DResourceDeleteParams{ID: id, TenantID: tenant})
	return err
}
func (s *Store) BindModel3DResource(ctx context.Context, tenantID string, c application.BindModel3DResource) error {
	id, tenant := c.TargetID, tenantID
	resource := nullString(c.ResourceID)
	var err error
	var n int64
	switch c.Kind {
	case "model":
