...
import (
	...
	dbq "github.com/SampsonFox/assetloop/internal/store/sqlite/sqlitedb"
	"time"
)
...
func (s *Store) LockMarketBaseCurrency(ctx context.Context, tenant, currency string) error {
	...
	return s.queries().LockTenantBaseCurrency(ctx, dbq.LockTenantBaseCurrencyParams{ID: tenant, BaseCurrency: currency})
}
