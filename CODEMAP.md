...
| `internal/store/storetest/multiple_purchases.go` | One built-in purchase per item (the acquisition) plus reusable custom expense types for services and accessories and a custom neutral gift; cost totals/grouping, earliest-acquisition duration, sale restrictions, custom post-sale costs, append-only reclassification (seeded legacy duplicate and zero legacy purchases, target-scope denials, last-acquisition guard, replay/conflict/concurrency) on both databases |
...
| `internal/store/storetest/lifecycle_safety.go`, `internal/web/lifecycle_retry_test.go` | Concurrent mutations (one effective acquisition and one effective sale), independent-connection retries, receipt rollback, and Web form replay; the Web suite also proves the unique built-in purchase, the positive amount rule and the custom neutral gift form |
...
| `internal/integration/full_element_test.go` | cumulative auth → persisted preferences → typed model allowances → direct items with different capacity tags sharing an appearance GLB → dedicated override/inheritance → foreign purchase → repair correction → sale → custom service/accessory costs, neutral gift and append-only reclassification scenario on both databases |
| `internal/integration/mcp_full_element_test.go` | Extends the same full-element scenario with HTTP Web consent/PKCE exchange, authenticated SDK discovery/write/retry/correction, custom cost recording and an optional-`type_id` reclassification refused for built-in targets, offline receipt-backed model-image import, bounded content upload and read with Web visibility, and client revocation without ending the Web session |
